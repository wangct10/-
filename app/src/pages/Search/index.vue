...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getData();
      // 地址栏需要改写，进行路由跳转(现在的路由跳转只是跳转到自己这里)
      //严谨：本意是想删除query，如果路径当中出现params，路由跳转应该带上params
    ...
    // 收集平台售卖属性地方回调函数（自定义事件）
    attrInfo(attr, attrValue) {
      console.log(attr, attrValue);
      // ["属性ID：属性值：属性名"]
      let props = `${attr.attrId}:${attrValue}:${attr.attrName}`;
      // 数组
      // 如果数据没有该数据就会返回-1，有则会返回索引位置
  ...
  beforeMount() {
    ...
    // 在发请求之前。把接口需要传递参数，进行整理（在给服务器发请求之前，把参数整理好，服务器会返回查询的数据）
  ...
  watch: {
    // 监听属性,监听路由是否发生变化，如果发生变化，在此发起请求
    $route(newValue, oldValue) {
      // 每一次请求完毕，应该把相应的1、2、3级分类的id置空，让他接受下一次的相应1、2、3id，不然就不会返回给你相应的数据页面

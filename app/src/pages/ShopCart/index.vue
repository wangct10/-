...
export default {
  ...
  methods: {
    ...
    async updateChecked(skuId, event) {
      ...
      try {
        ...
        // 如果修改成功，再次获得服务器数据（购物车）
        let isChecked = event.target.checked ? "1" : "0";
        await this.UpdateCheckById({ skuId, isChecked });
        this.getData();
      } catch (error) {
        alert(error.message);
      }
  ...
  computed: {
    ...
    //判断底部全选复选框是否勾选【全部产品都选中，采勾选】
    isAllCheck() {
      // 遍历数组里面的原理，只要全部元素isChecked属性都为1==》真 true
      //只要有一个不是1，==》假false

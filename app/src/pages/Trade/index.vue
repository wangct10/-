  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="address1 in address"
        :key="address1.id"
      >
        <span class="username" :class="{ selected: address1.isDefault == 1 }">{{
          address1.consignee
        }}</span>
        <p @click="changeDefault(address1, address)">
          <span class="s1">{{ address1.fullAddress }}</span>
          <span class="s2">{{ address1.phoneNum }}</span>
          <span class="s3" v-show="address1.isDefault == 1">默认地址</span>
        </p>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions("trade", ["getUserAddress", "getOrderInfo"]),
    // 修改默认地址
    changeDefault(address1, address) {
      // 全部的isDefault为0
      address.forEach((item) => {
        item.isDefault = 0;
      });
      address1.isDefault = 1;
    },
    // 提交订单
    async submitOrder() {
      // 交易编码

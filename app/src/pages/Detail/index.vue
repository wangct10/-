        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="supSaleAttr in supSaleAttrList" :key="supSaleAttr.id">
                <dt class="title">{{ supSaleAttr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: supSaleAttrValue.isChecked == 3 }"
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们路由跳转 从A路由跳转到B路由，这里再加入购物车，进行路由跳转之前 ，发请求，把你购买的产品的信息通过请求形式通知服务器，服务器进行相应的存储-->
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      /* 当前这里是派发一个action，也向服务区发请求，判断加入购物车成功还是失败了，{
        skuId: this.$route.params.skuid,
        skuNum: this.skuNum,
      }
      上面代码说白了，调用仓库中的addOrUpdateShopCart,
      这个方法加上async，返回的一定是Promise
      要么成功 要么失败
      */
      //Promise.prototype.catch()
      // 2.服务区存储成功---进行路由跳转.失败则提示
      try {
        let result = await this.addOrUpdateShopCart({
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        // 3：路由跳转
        // 4：在路由跳转的时候还需要讲产品信息带给下一级路由组件
        // 下面这种手段路由跳转一级可以传递参数
  ...
  computed: {
    ...mapGetters("detail", ["categoryView", "skuInfo", "supSaleAttrList"]),
    // 给子组件的数据
    skuImageList() {
